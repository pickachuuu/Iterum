...
export default function InterviewCarousel() {
  ...
  return (
    <section id="rounds" className="relative py-28 bg-white overflow-hidden border-y border-slate-100">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden select-none">
        <div 
          className="absolute -top-1/4 -right-1/4 w-[600px] h-[600px] rounded-full blur-3xl opacity-20 transition-all duration-1000"
          style={{ backgroundColor: activeRound.glowColor }}
        />
        <div 
          className="absolute -bottom-1/4 -left-1/4 w-[600px] h-[600px] rounded-full blur-3xl opacity-25 transition-all duration-1000"
          style={{ backgroundColor: activeRound.glowColor }}
        />
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#f1f5f9_1px,transparent_1px),linear-gradient(to_bottom,#f1f5f9_1px,transparent_1px)] bg-[size:32px_32px] opacity-40 [mask-image:radial-gradient(ellipse_60%_50%_at_50%_50%,#000_70%,transparent_100%)]" />
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 z-10">
        ...
        <div className="max-w-4xl mb-20 text-left">
          <span className="inline-flex items-center gap-2 text-xs font-extrabold text-brand-600 bg-brand-50 border border-brand-100/50 px-3 py-1 rounded-full uppercase tracking-widest">
            <Compass className="h-4.5 w-4.5 text-brand-500 animate-spin" style={{ animationDuration: '6s' }} /> Segmented Practice Pathways
          </span>
          <h2 className="font-display text-4xl sm:text-6xl font-black text-slate-900 tracking-tight mt-4 leading-none">
            An interactive compass of <br />
            <span className="bg-gradient-to-r from-brand-600 to-accent-rose bg-clip-text text-transparent">your career milestone rounds</span>
          </h2>
          <p className="mt-6 text-lg sm:text-xl text-slate-550 leading-relaxed font-sans font-medium max-w-3xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
          ...
          <div className="lg:col-span-5 flex items-center justify-center relative py-8">
            ...
            <div className="relative w-[340px] h-[340px] md:w-[400px] md:h-[400px] rounded-full border border-slate-100 bg-white shadow-2xl flex items-center justify-center overflow-hidden">
              
              {/* Decorative slow-spinning dashed background ring */}
              <div className="absolute inset-2.5 rounded-full border border-dashed border-slate-200 animate-slow-spin pointer-events-none opacity-60" />
              
              {/* Rotating Node Tracker Wrapper (Main spinning interaction!) */}
              <div 
                      ...
                      >
                        ...
                        {isActive && (
                          <div 
                            ...
                            style={{
                              background: `radial-gradient(circle, ${activeRound.glowColor} 0%, transparent 70%)`
                            }}
                          />
                        )}
                        ...
                        <div className={`h-13 w-13 md:h-15 md:w-15 rounded-full flex flex-col items-center justify-center border font-sans font-bold transition-all duration-300 ${
                          isActive
                            ? `bg-gradient-to-tr ${round.colorClass} text-white shadow-lg shadow-slate-900/10 scale-110 border-transparent`
                            : 'bg-white border-slate-200 text-slate-600 hover:border-slate-400 hover:text-slate-900 shadow-sm hover:scale-105'
                        }`}>
                        ...
                        {/* Hover tooltips */}
                        <span className={`absolute top-15 md:top-17 left-1/2 -translate-x-1/2 whitespace-nowrap text-[9px] font-extrabold uppercase tracking-wider px-2 py-0.5 bg-slate-900 text-white rounded shadow-xs transition-all duration-300 pointer-events-none ${
                          isActive 
              ...
              <div className="relative h-24 w-24 md:h-28 md:w-28 rounded-full bg-slate-50 border border-slate-100 shadow-md flex flex-col items-center justify-center z-10 select-none">
                <div className="h-8 w-8 text-slate-800"><LogoSvg /></div>
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1.5">ITERUM CORE</span>
              </div>
          ...
          {/* THE MASTERCLASS FRONTEND PRACTICE FOR TABS (CSS Grid Overlap): 
              We map all panels to grid column 1, row 1. The grid container naturally 
              adjusts its static height to the tallest panel on any viewport size, 
              guaranteeing zero layout height shift when switching tabs while ensuring 
              absolutely no text cuts off under any screen scaling! */}
                  ...
                  >
                    <div className="space-y-6">
                      ...
                      <div className="relative">
                        <span className="absolute -top-16 -left-4 text-[100px] sm:text-[130px] font-black text-slate-100/70 select-none pointer-events-none font-mono leading-none tracking-tight">
                          {round.num}
                        </span>
                        
                        <div className="relative pt-4 space-y-2">
                          <span className="inline-block text-xs font-bold text-brand-600 bg-brand-50 border border-brand-100/50 px-3 py-1 rounded-full uppercase tracking-widest">
                            VIBE: {round.vibe}
                          </span>
                          
                          <h3 className="font-display text-3xl sm:text-5xl font-black text-slate-900 tracking-tight pt-1.5">
                            {round.title}
                          </h3>
                        </div>
                      </div>
                      ...
                      {/* Literary Soul Quote */}
                      <blockquote className="border-l-4 border-slate-900 pl-5 py-1.5">
                        <p className="text-base sm:text-lg text-slate-655 font-sans font-medium italic leading-relaxed">
                          "{round.soulQuote}"
                        </p>
                      </blockquote>
                      ...
                      <div className="space-y-5">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 border-b border-slate-100 pb-2">
                          <Sparkles className="h-3.5 w-3.5 text-brand-500" /> Key Strategic Focus Objectives
                        </h4>
                        
                        <div className="grid grid-cols-1 gap-5">
                          {round.highlights.map((h, i) => (
                            <div key={i} className="group flex items-start gap-4">
                              <span className="h-6.5 w-6.5 rounded bg-slate-50 border border-slate-200 text-slate-500 text-[10px] font-mono font-bold flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors group-hover:bg-slate-900 group-hover:text-white group-hover:border-slate-900">
                                0{i + 1}
                              </span>
                              <div className="space-y-0.5">
                                <p className="font-sans font-extrabold text-sm text-slate-900 tracking-tight group-hover:text-brand-600 transition-colors">
                                  {h.title}
                                </p>
